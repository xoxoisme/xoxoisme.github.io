...
- 발급이 되면 서버에 저장되지 않고 토큰 자체로 검증하며 사용자 권한을 인증한다.
- 이로 인해, 탈취되면 토큰을 획득한 누구나 권한 접근이 가능해지는 문제가 발생한다.

## Refresh Token
- 그래서, Access Token의 유효기간은 짧게 Refresh Token의 유효기간은 보다 길게 설정한다.
- Acess Token이 접근에 관여하는 토큰, Refresh Token은 재발급에 관여하는 토큰이다.
