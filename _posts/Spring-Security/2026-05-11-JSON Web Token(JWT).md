...
- 브라우저가 쿠키를 자동으로 전송하는 특성을 이용한 해킹 공격이다.
- Jwt를 쓰면 쿠키가 아닌 `Authorization` 헤더로 전송하기에 비활성화한다.
- 다만, Jwt를 쿠키에 저장하는 방식을 쓴다면 비활성화 하면 안된다.
- 보안은 항상 트레이드 오프이기에 각 상황에 맞게 사용하면 된다.

#### CORS

- 다른 출처(Origin)의 요청을 브라우저가 차단하는 보안 정책이다.
...
- 로그인 시 클라이언트에게 `accessToken`을 전송하기 위해 토큰을 생성하고 검증하는 역할을 한다.
