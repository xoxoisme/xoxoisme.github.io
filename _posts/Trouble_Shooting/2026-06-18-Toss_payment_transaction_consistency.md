...
```
[FE] POST /api/v1/payments          → Payment(PENDING) 저장, orderId 발급
[FE] 토스 결제창에서 사용자 결제      → 토스가 successUrl로 paymentKey 전달
[FE] POST /api/v1/payments/confirm  → 백엔드가 토스에 "진짜 승인" 호출 → 이때 돈 빠짐
```

- 즉, 승인(confirm) 시점에 2가지 **외부/내부 작업이 연달아서** 발생한다.
    1. 토스 서버에 승인 요청(성공하면 실제로 사용자 돈이 빠져나감)
